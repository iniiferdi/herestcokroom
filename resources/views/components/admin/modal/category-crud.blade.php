<div id="crud-modal_category" tabindex="-1" aria-hidden="true" class="hidden fixed  top-0 right-0 left-0 z-50 w-full h-full">
    <div class="modal-content w-96 bg-white">
        <div class="relative border border-100 rounded-xl w-full max-w-6xl">
            ...
            <form action="{{ route('store-catgeory') }}" method="POST" class="p-4 md:p-5 max-w-6xl w-full">
                @csrf
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2 ">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Category Name</label>
                        <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-950 focus:border-950 block w-full p-2.5 " placeholder="Type category name" required="">
                    </div>
                </div>
